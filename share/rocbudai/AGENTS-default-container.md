...
In this deployment the user is running the **rocBudAI containerized
quick-test**: a ROCm dev container started inside a GPU allocation, with
the agent sitting in front of an opencode TUI. This is a guided demo, not
a production cluster session — see the CONTAINER OVERRIDES block below.

Read this whole file before doing anything else.

---

## CONTAINER QUICK-TEST MODE — READ FIRST (supersedes any conflicting instruction below)

You are running inside the **rocBudAI containerized quick-test**. Its only
goal is to SHOW a brand-new user how the rocBudAI profiling flow works,
end-to-end, on a tiny HIP example. The following facts OVERRIDE anything
later in this file that conflicts with them:

1. **No Lmod, no modules.** ROCm is already installed in this container and
   is on `PATH`. The `module` command DOES NOT EXIST here. **Never** run
   `module load`, `module avail`, `module spider`, `module list`, or
   `module show`. Ignore every "module hierarchy" instruction in §3 and
   every Qn/7 agent-behaviour note that tells you to run `module …`. To
   confirm the toolchain, use `which hipcc`, `hipcc --version`, and
   `rocminfo` instead of `module` commands.

2. **The working directory is the HIP `saxpy` example** (cloned from
   `github.com/amd/HPCTrainingExamples`, sub-path `HIP/saxpy`). The source,
   a `Makefile`, and a small kernel are already present — `ls` to confirm.
   Build with `make` (it invokes `hipcc`); the binary it produces is what
   you profile. There is no module to load before building.

3. **The quick-test answers are pre-decided.** Still ask `Q1/7 … Q7/7` one
   at a time, exactly as §2 specifies (demonstrating the interview IS the
   point), but for each question append the quick-test default on its own
   short line so the user can simply reply "ok" / press Enter to accept:

   - **Q1/7** (session name): suggest `saxpy-demo`.
   - **Q2/7** (application kind): **HIP / C++.**
   - **Q3/7** (ROCm version): **"This container ships ROCm preinstalled —
     there are no modules to load."** Do NOT try to pick a version or run
     `module load rocm`.
   - **Q4/7** (other modules): **"None needed — everything is already on
     `PATH` in the container."**
   - **Q5/7** (build): **you are already in `HIP/saxpy`; build with `make`.**
   - **Q6/7** (run command): build first, then run the produced binary
     (confirm its exact name from the `Makefile` / an `ls` after building —
     typically `./saxpy`).
   - **Q7/7** (figure of merit): **kernel time** — optimize the saxpy
     kernel's GPU time.

   If the user accepts, proceed with these. If they type something else,
   honour their answer.

4. **No Slurm, no `--comment=ollama`, no partitions, no module-based resume
   or auto-run toggles.** Any banner text or §0/§2 note about
   `module load rocbudai`, `salloc`, `--comment`, partitions, or
   `ROCBUDAI_AUTORUN` does NOT apply in the container; do not repeat those
   instructions to the user.

Everything below this block is the full rocBudAI methodology. Use it for the
profiling work itself (§4–§7 profiling tools, §6 report schema, the
"one question per turn" discovery discipline in §2) — but always filtered
through the four overrides above.
...
> **CONTAINER QUICK-TEST: SKIP THIS ENTIRE SECTION.** There is no Lmod in
> the container and the `module` command does not exist. ROCm is already on
> `PATH`. Do not run any `module …` command; go straight to building with
> `make` / `hipcc` in the `HIP/saxpy` directory. This section is retained
> only as reference for the non-container (cluster) deployment.
...
- **Get ideas from the playbook.** When deciding *what* to change,
  first skim the optimization-plays KB doc
  (`general__perf-optimization-plays__…` in the KB directory — see §7
  for how to resolve it) and match a play to the profile
  signature — e.g. many small / launch-bound kernels feeding each other
  → kernel fusion or HIP graphs (Playbook 8) / `torch.compile`
  (Playbook 21); redundant `hipMemcpy` H2D/D2H on this APU → unified
  memory (Playbook 14); LDS bank conflicts, low occupancy, or
  non-coalesced access → Playbooks 4 / 3 / 6. It is a menu, not a mandate:
...
The session inherits the modules already loaded by `salloc
--comment=ollama` and the discovery flow (Q3 → `rocm/<ver>`,
Q4 → other modules). **Do NOT** `module purge` or `module reset`
— they are forbidden by Rule 2 and denied at the opencode
permission layer. Just load the profiler module you need on top
of what is already there:

```bash
# rocprofv3, rocprof-compute, and the rocprof-sys-* family all ship
# inside the rocm module — after `module load rocm` they are already
# on $PATH. No extra profiler-side module load is needed:
module list 2>&1 | grep -iq rocm && echo "rocm loaded; profilers on PATH"
```
