<!--
  ARCH PERSONA — gfx90a (CDNA2: AMD Instinct MI210 / MI250 / MI250X)
  Derived from the gfx942 + MI300A reference persona
  (share/rocbudai/AGENTS-default.md), with the hardware facts replaced
  by CDNA2 values. All peak-theoretical numbers below are taken verbatim
  from AMD's "Introducing AMD CDNA 2 Architecture" whitepaper and the
  AMD Instinct MI200-series product table; cite them as peak theoretical
  and verify against the vendor datasheet before quoting in a report.
-->
...
7. **Sanity-check numbers.** Reasonable orders of magnitude on a modern
   ...
   **Canonical MI250X peak-theoretical spec table (use these numbers
   verbatim — never guess, never "round" from memory). Values are per
   OAM package (= 2 GCDs); CDNA2 is dual-die and ROCm exposes each GCD
   as a SEPARATE device, so per-GCD numbers are half of these:**

   | Quantity                       | Value                       | Notes                                   |
   |--------------------------------|-----------------------------|-----------------------------------------|
   | FP64 vector                    | **47.9 TFLOPS**             | peak theoretical, per package           |
   | FP32 vector                    | **47.9 TFLOPS**             | peak theoretical (95.7 packed FP32)     |
   | FP64 matrix                    | **95.7 TFLOPS**             | CDNA2 headline feature (FP64 MFMA)      |
   | FP32 matrix                    | **95.7 TFLOPS**             | peak theoretical                        |
   | FP16 / BF16 matrix             | **383.0 TFLOPS**            | peak theoretical                        |
   | INT8 / INT4                    | **383.0 TOPS**              | peak theoretical                        |
   | HBM2e capacity                 | **128 GB**                  | 64 GB per GCD                           |
   | HBM2e bandwidth                | **3.2 TB/s**                | peak theoretical (~1.6 TB/s per GCD)    |
   | HBM2e interface width          | **8192 bits**               | 4096 bits per GCD                       |
   | L2 cache                       | **8 MB per GCD**            | 16-way, 32 slices; NO Infinity Cache    |
   | CDNA2 compute units            | **220**                     | 110 active per GCD                       |
   | Matrix cores                   | **880**                     | 440 per GCD                             |
   | Stream processors              | **14,080**                  | 7,040 per GCD                           |
   | Peak engine clock              | **1700 MHz**                | per package                             |
   | TDP                            | **500 W / 560 W**           | OAM (passive / liquid)                  |

   Source: AMD "Introducing AMD CDNA 2 Architecture" whitepaper and the
   AMD Instinct MI200-series product table. Sibling parts: **MI250**
   (208 CU, 128 GB, 45.3 / 90.5 TFLOPS FP64 vec/matrix, 500/560 W) and
   **MI210** (single GCD, 104 CU, 64 GB, 22.6 / 45.3 TFLOPS FP64
   vec/matrix, 1.6 TB/s, 300 W). When you cite a peak number in a
   report, name it as **peak theoretical** so the user can verify
   ...
   **not** in the table above, **not** from a real measurement you
   ...
   1. Quote the exact number from AMD's published MI200-series
      datasheet or the CDNA2 architecture whitepaper (peak FLOPS, HBM
      specs, L2, TDP, Infinity Fabric link counts, GCD layout), or
      from a KB sidecar you `Read` this session.
   2. Tell the user you do not know the number and ask whether
      they have a reference. **Do not invent.**

   Common confusions to avoid:

   - **CDNA2 is dual-GCD.** A single MI250X OAM shows up as **2 HIP
     devices**, each with 110 CUs and 64 GB. Do not report per-GCD
     numbers as if they were the whole package, or vice versa. A
     "single MI250X" job is effectively a 2-GPU job; GCD-to-GCD goes
     over in-package Infinity Fabric (~400 GB/s bi-directional).
   - **No FP8, no TF32, no MX microscaling, no structured sparsity.**
     These are CDNA3/CDNA4 features. CDNA2 MFMA covers FP64, FP32,
     FP16, BF16, INT8/INT4 only. If a kernel or report mentions FP8 /
     MXFP4 / `global_load_lds` on gfx90a, it is wrong for this arch.
   - **FP64 matrix is the headline CDNA2 feature** (16×16×4 and 4×4×4
     MFMA), giving 95.7 TFLOPS — 2× the FP64 vector rate. HPC FP64
     matmul should use it.
   - **Discrete GPU, not an APU.** Use explicit host/device copies
     (`hipMemcpy`). Cache-coherent CPU↔GPU shared memory exists ONLY
     on the MI250X + optimized 3rd-Gen EPYC configuration (e.g.
     Frontier-class nodes, with `HSA_XNACK=1`); plain PCIe MI250/MI210
     have no coherent unified memory.
   - LDS is **64 KB per CU** (same as CDNA3), not the 160 KB of CDNA4.
...
- **Get ideas from the playbook.** When deciding *what* to change,
  first skim the **MI250X** optimization-plays KB doc
  (`general__perf-optimization-plays-mi250x__…` in the KB directory —
  ...
  → kernel fusion (Playbook 9) / HIP graphs (Playbook 10); FP8/MX code
  paths that must fall back to BF16 on CDNA2 (Playbooks 6 / 46); both
  GCDs used and correctly affinitized (Playbook 18); LDS bank conflicts,
  low occupancy, or non-coalesced access → Playbooks 4 / 3 / 7. It is a
  menu, not a mandate:
...
| `rocprof-compute`| "Why is *this kernel* slow? Roofline / percent-of-peak, occupancy limiters / SPI / wavefront launch stats" — per-kernel deep dive **and** roofline analysis. **Caveat:** known pandas/numpy compat error on this cluster's ROCm-7.2.x ML overlay (`pytorch/2.9.1*` modules); see `### PyTorch / TensorFlow / JAX` below | `rocprof-compute profile -n <name> -- ./app` then `analyze -p workloads/<name>/MI200_A1` |
...
- **rocprof-compute** writes `workloads/<name>/MI200_A1/` and is read
...
rocprof-compute analyze -p workloads/<name>/MI200_A1

# compare two versions (problem vs solution)
rocprof-compute analyze \
    -p workloads/v1/MI200_A1 -p workloads/v2/MI200_A1 \
...
2. **Then — `rocprof-compute`** — roofline / percent-of-peak plus a
   ...
   rocprof-compute analyze -p workloads/ml_run/MI200_A1
   ```
