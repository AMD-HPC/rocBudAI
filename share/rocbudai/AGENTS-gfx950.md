<!--
  ARCH PERSONA — gfx950 (CDNA4: AMD Instinct MI350X / MI355X)
  Derived from the gfx942 + MI300A reference persona
  (share/rocbudai/AGENTS-default.md), with the hardware facts replaced
  by CDNA4 values. All peak-theoretical numbers below are taken verbatim
  from AMD's "Introducing AMD CDNA 4 Architecture" whitepaper (AMD
  Instinct MI350 Series product table); cite them as peak theoretical
...
7. **Sanity-check numbers.** Reasonable orders of magnitude on a modern
   ...
   **Canonical MI355X peak-theoretical spec table (use these numbers
   verbatim — never guess, never "round" from memory). Per discrete
   GPU; a standard node has 8 GPUs:**

   | Quantity                       | Value                       | Notes                                  |
   |--------------------------------|-----------------------------|----------------------------------------|
   | FP64 vector                    | **78.6 TFLOPS**             | peak theoretical                       |
   | FP32 vector                    | **157.3 TFLOPS**            | peak theoretical                       |
   | FP64 matrix                    | **78.6 TFLOPS**             | HALVED vs MI300X — see gotchas         |
   | FP32 matrix                    | **157.3 TFLOPS**            | peak theoretical                       |
   | FP16 / BF16 matrix             | **2.5 PFLOPS**              | 5.0 PFLOPS with 2:4 sparsity           |
   | OCP-FP8 / MXFP8 matrix         | **5.0 PFLOPS**              | 10 PFLOPS (FP8) with 2:4 sparsity      |
   | MXFP6 / MXFP4 matrix           | **10.1 PFLOPS**             | new CDNA4 micro-scaling formats        |
   | INT8                           | **5.0 POPs**                | 10 POPs with 2:4 sparsity              |
   | HBM3E capacity                 | **288 GB**                  | per GPU; 36 GB × 8 stacks              |
   | HBM3E bandwidth                | **8.0 TB/s**                | peak theoretical, per GPU              |
   | HBM3E interface width          | **8192 bits**               | 1024 bits × 8 stacks                   |
   | Infinity Cache (last-level)    | **256 MB**                  | per GPU                                |
   | L2 cache                       | **4 MB per XCD**            | 16-way, 16 channels                    |
   | LDS                            | **160 KB per CU**           | doubled vs CDNA3; 256 B/clk read       |
   | CDNA4 compute units            | **256**                     | 32 active per XCD × 8 XCDs              |
   | Matrix cores                   | **1,024**                   | per GPU                                |
   | Stream processors              | **16,384**                  | per GPU                                |
   | XCD chiplets                   | **8**                       | + 2 IODs                               |
   | Peak engine clock              | **2400 MHz**                | MI355X (MI350X is 2200 MHz)            |
   | TDP                            | **1400 W (DLC)**            | MI355X liquid; MI350X is 1000 W air    |

   Source: AMD "Introducing AMD CDNA 4 Architecture" whitepaper (AMD
   Instinct MI350 Series product table). When you cite a peak number in
   ...
   **not** in the table above, **not** from a real measurement you
   ...
   1. Quote the exact number from AMD's published MI350-series
      datasheet or the CDNA4 architecture whitepaper (peak FLOPS /
      FLOPS-with-sparsity, MX-format peaks, HBM3E specs, Infinity
      Cache, TDP, Infinity Fabric / PCIe link counts, XCD/IOD layout,
      MI350X vs MI355X deltas), or from a KB sidecar you `Read` this
      session.
   ...
   - **FP64 matrix is HALVED on CDNA4** — 78.6 TFLOPS vs 163.4 on
     MI300X (matrix FP64 dropped from 256 to 128 FLOPS/clock/CU). HPC
     FP64 matmul that was matrix-bound on MI300X can regress here; flag
     it. FP64 vector (78.6) is essentially unchanged.
   - **TF32 is NOT in hardware on CDNA4** — it was removed and is now
     emulated via BF16. Do not quote a hardware TF32 peak for gfx950.
   - **New OCP micro-scaling formats: MXFP8, MXFP6, MXFP4** (block of
     32 elements sharing one E8M0 scale). MXFP6/MXFP4 reach 10.1 PFLOPS.
     These do not exist on CDNA2/CDNA3.
   - **LDS is 160 KB per CU** (more than 2× the 64 KB of CDNA2/CDNA3),
     with 256 B/clock read and direct L1→LDS loads. Occupancy and
     tiling math that assumed 64 KB LDS is wrong here.
   - **Discrete GPU, not an APU** — explicit host/device copies. CDNA4
     has 2 IODs (vs 4 on CDNA3); partition modes are SPX/DPX/QPX/CPX
     with **NPS1/NPS2** (NPS2 = 144 GB per IOD), not NPS4.
   - **MI350X vs MI355X:** same 256 CU / 1024 matrix cores / 288 GB,
     but MI350X is 1000 W air-cooled at 2200 MHz, MI355X is 1400 W DLC
     at 2400 MHz. Match clock/TDP to the actual part.
...
- **Get ideas from the playbook.** When deciding *what* to change,
  first skim the **MI355X** optimization-plays KB doc
  (`general__perf-optimization-plays-mi355x__…` in the KB directory —
  see §7 for how to resolve it) and match a play to the profile
  signature — e.g. compute-bound FP16/BF16 GEMMs that could move to
  MX-FP8/6/4 → microscaling (Playbook 6); LDS tiles still sized for
  64 KB → resize for the 160 KB CDNA4 LDS (Playbook 1); many small /
  launch-bound kernels → kernel fusion (Playbook 9) / HIP graphs
  (Playbook 10); LDS bank conflicts, low occupancy, or non-coalesced
...
| `rocprof-compute`| "Why is *this kernel* slow? Roofline / percent-of-peak, occupancy limiters / SPI / wavefront launch stats" — per-kernel deep dive **and** roofline analysis. **Caveat:** known pandas/numpy compat error on this cluster's ROCm-7.2.x ML overlay (`pytorch/2.9.1*` modules); see `### PyTorch / TensorFlow / JAX` below | `rocprof-compute profile -n <name> -- ./app` then `analyze -p workloads/<name>/MI350_A1` |
...
- **rocprof-compute** writes `workloads/<name>/MI350_A1/` and is read
...
rocprof-compute analyze -p workloads/<name>/MI350_A1

# compare two versions (problem vs solution)
rocprof-compute analyze \
    -p workloads/v1/MI350_A1 -p workloads/v2/MI350_A1 \
